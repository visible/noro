...
function CategoryCard({
	...
	return (
		<div className="bg-[#161616] rounded-xl overflow-hidden border border-white/5">
			<button
				onClick={onToggle}
			...
			{expanded && items.length > 0 && (
				<div className="border-t border-white/5">
					{items.map((item, i) => (
						<div
							key={item.id}
							className={`px-5 py-3 flex items-center justify-between ${i !== items.length - 1 ? "border-b border-white/5" : ""}`}
						>
							<div>
								<p className="font-medium text-white">{item.title}</p>
								<p className="text-sm text-white/40">{item.message}</p>
							</div>
							<button
								onClick={() => onFix(item.itemId)}
								className="px-3 py-1.5 text-sm font-medium text-[#d4b08c] bg-[#d4b08c]/10 rounded-lg hover:bg-[#d4b08c]/20 transition-colors"
							>
								fix
							</button>
						</div>
...
export default function Watchtower() {
	...
	if (loading) {
		return (
			<Page>
				<Header title="watchtower" description="analyzing your vault security..." />
				<Loading text="scanning for vulnerabilities..." />
			</Page>
		);
	}
	...
	if (!report) {
		return (
			<Page>
				<Header title="watchtower" description="security monitoring dashboard" />
	...
	return (
		<Page>
			...
			<div className="bg-[#161616] rounded-xl p-8 mb-8 border border-white/5">
					...
					<ul className="space-y-2">
						{report.recommendations.map((rec, i) => (
							<li key={i} className="flex items-start gap-3">
								<svg
									className="w-5 h-5 text-[#d4b08c] shrink-0 mt-0.5"
									viewBox="0 0 24 24"
									fill="none"
									stroke="currentColor"
									strokeWidth="2"
