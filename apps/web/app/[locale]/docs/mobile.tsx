	...
	return (
		<>
			...
			>
				...
				<div
					className={`absolute top-0 left-0 right-0 bg-[#141414] shadow-2xl rounded-b-2xl transition-transform duration-300 ease-out border-b border-white/10 ${
						open ? "translate-y-0" : "-translate-y-full"
					}`}
				>
					<div className="flex items-center justify-between p-4 border-b border-white/5">
						<Link href="/" className="flex items-center gap-2" onClick={() => setOpen(false)}>
							<div className="w-8 h-8 rounded-lg bg-[#d4b08c] flex items-center justify-center">
								<svg className="w-5 h-5 text-[#0a0a0a]" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
					...
					<nav className="p-4 pb-6 max-h-[70vh] overflow-y-auto">
						<div className="space-y-4">
							{navigation.map((group) => (
								<div key={group.title}>
									<h4 className="text-[11px] font-semibold uppercase tracking-wider text-[#d4b08c]/70 mb-2 px-3">
										{group.title}
									</h4>
